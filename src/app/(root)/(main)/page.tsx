...
export default async function Home() {
  ...
  const [reports, rewards, tasks] = await Promise.all([
    getRecentReports(100).catch(() => []), // Handle errors gracefully
    getAllRewards().catch(() => []),
  ...
  const wasteCollected =
    tasks?.reduce((total, task) => {
      const amount = Number(task?.amount?.match(/\d+(\.\d+)?/)?.[0]) || 0;
      return total + amount;
    }, 0) || 0;

  const reportsSubmitted = reports?.length || 0;
  const tokensEarned = rewards?.reduce((total, reward) => total + (reward?.points || 0), 0) || 0;
    ...
    <div className={`container mx-auto px-4 py-16 `}>
      <section className="text-center mb-20">
        ...
        <h1 className="text-6xl font-bold mb-6 text-gray-800 tracking-tight">
          Eco Vision <span className="text-green-600">Waste Management</span>
        </h1>
